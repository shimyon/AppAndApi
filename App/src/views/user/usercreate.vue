          ...
          <v-form>
            <v-container py-0>
              <v-layout wrap>
                ...
                <v-flex xs12 md6>
                  <v-text-field label="Email Address" v-model="UserDetails.Email" class="purple-input" />
                </v-flex>
                <v-flex xs12 md6>
                  <v-text-field label="First Name" v-model="UserDetails.FirstName" class="purple-input" />
                </v-flex>
                <v-flex xs12 md6>
                  <v-text-field label="Last Name" v-model="UserDetails.SurName" class="purple-input" />
                </v-flex>
                <v-flex xs12>
                  <v-textarea
                ...
                <v-flex xs12 text-xs-right>
                  <v-btn class="mx-0 font-weight-light" @click="UpdateProfile()" color="success">Update Profile</v-btn>
                  &nbsp;<v-btn class="mx-0 font-weight-light" :to="'/user/userlist'" color="danger">Cancel</v-btn>
                </v-flex>
              </v-layout>
            </v-container>
          </v-form>
...
export default {
  ...
  methods: {
    ...
    UpdateProfile() {
      let actionUrl = "User/Create";
      if (this.$route.params.id) {
        actionUrl = `User/Edit?tblId=${this.$route.params.id}`;
      }
